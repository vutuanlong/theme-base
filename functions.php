...
if ( ! function_exists( 'themebase_setup' ) ) :
	...
	function themebase_setup() {
		...
		register_nav_menus(
			array(
				'menu-1' => esc_html__( 'Primary', 'themebase' ),
			)
		);
		...
		add_theme_support(
			'custom-background',
			apply_filters(
				'themebase_custom_background_args',
				array(
...
function themebase_scripts() {
	wp_enqueue_style( 'themebase-style', get_stylesheet_uri(), array(), _S_VERSION );
	wp_style_add_data( 'themebase-style', 'rtl', 'replace' );

	wp_enqueue_script( 'themebase-navigation', get_template_directory_uri() . '/js/navigation.js', array(), _S_VERSION, true );
